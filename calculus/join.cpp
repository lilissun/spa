...
namespace cal {
    ...
    void Join::execute(Program *program, const State *state, std::vector<tim::Rule *> &rules) const
    {
        ...
        auto join_event = program->join_event(_name->name(), next_state->get_identity()->clone(), arguments, timestamp);
        next_state->add_premise(join_event);

        _next_process->execute(program, next_state, rules);
        delete next_state;
    }
    
    void Join::info(const size_t depth, std::ostream &os) const
    {
        os << com::tabs(process_tab_size, depth) << process_engage << process_space;
        os << process_join << process_space;
        _name->info(os);
        _arguments->info(os);
        os << process_at;
        _timestamp->info(os);
